...
public class Bandit : MonoBehaviour, EnemyDetector.EnemyDetectorCallback {
    ...
    private float                   m_attackRangeMultiplier = 3f;
    ...
    private void Hit(){
        if(Vector2.Distance(m_objectToChase.position, transform.position) > attackRange * m_attackRangeMultiplier * 2){
    ...
    private void Run(){
        ...
        if(m_objectToChase != null){
            var distance = Mathf.Sqrt(Mathf.Pow(transform.position.x - m_objectToChase.position.x, 2));
            Debug.Log(distance);
            if(distance <= attackRange * m_attackRangeMultiplier){
                StopAndHit();
            }
        }
    }
